...
- [x] **Linha de visão automática** — mover um token do tipo "Jogador" num mapa de tiles com névoa ativa revela automaticamente as células num raio de 3 ao redor da nova posição (círculo simples, sem considerar paredes bloqueando visão de verdade). O pincel manual de névoa continua disponível pra revelar áreas que o grupo ainda não visitou fisicamente (o que se vê de longe, por exemplo). Sem migration — só lógica de cliente sobre o `tile_data.fog` que já existia.
