...
- [x] **Correção de inputs vazando pra célula vizinha** — `<input>`/`<select>`/`<textarea>` têm uma largura mínima intrínseca do navegador (~170-220px) que vencia o `flex:1`/`minmax()` do layout ao redor, então um campo numa grade estreita "vazava" visualmente por cima da coluna vizinha. Ficou óbvio na grade densa de Subatributos do Blade Strands (24 campos curtos com botão de dado cada), mas o mesmo bug existia em qualquer grid/flex apertado o bastante em qualquer ficha. Corrigido globalmente com `min-width: 0`.
- [x] **Conteúdo pronto pro Blade Strands** — Catálogo, Bestiário e Personagens prontos ganharam exemplos maiores pra esse sistema: `blade-strands.catalog.json` foi de ~9 pra 29 entradas (habilidades cobrindo as quatro categorias — Física/Estamina, Espiritual/Spirit, Mental/Mana, Existencial/Soul — e itens de suporte), e dois arquivos novos com o formato de import do Bestiário: `blade-strands.bestiary.json` (6 inimigos, de um "Eco Distorcido" fraco até o chefe "Núcleo do Colapso", cada um com atributos e Pilares de Ascensão já calculados) e `blade-strands.pregens.json` (4 personagens prontos pra jogar, com subatributos batendo a regra de soma do sistema). Os dois últimos aparecem como botões de exemplo dentro de "Importar moldes (JSON)" no Bestiário. De quebra, corrigido um bug real no validador de import do Bestiário: ele esperava `current`/`max` num recurso tipo barra, mas o app usa `atual`/`max` (e `texto` pro tipo texto) — um import antes deixava os recursos silenciosamente zerados sem dar erro nenhum.
- [x] **Alinhamento da grade de Subatributos** — campos com nome longo (ex: "Endurecimento (Resistência)") quebravam em 2 linhas enquanto os vizinhos na mesma fileira ficavam em 1, deixando os inputs desalinhados entre si. A seção ganhou uma classe própria (`section-<key>` no `SheetFieldsEditor`), coluna um pouco mais larga (reduz quebra) e altura de rótulo reservada (mantém alinhamento mesmo quando ainda quebra). Sem migration, só CSS.
- [x] **Duplicar personagem e molde** — botão "Duplicar" no Bestiário (cria uma cópia do molde, mesma ficha/habilidades/itens) e no card de cada personagem dentro de uma campanha (Mestre only — cria um NPC novo com a mesma ficha, copiando `character_abilities`/`inventory_items` associados). Sem migration.
...
- [x] **Restaurar e clonar campanha** — "Importar campanha (JSON)" no Dashboard recria uma campanha nova a partir de um backup baixado em "Baixar backup (JSON)"; "Clonar" em cada campanha que você é Mestre faz o mesmo direto dos dados ao vivo, sem passar por arquivo. Os dois alimentam a mesma função (`restoreCampaignFromSnapshot`): sistema, personagens (com habilidades/itens/segredos), mapas de tiles, Notas do Mestre e handouts (texto) são recriados; **é uma restauração parcial** — mapas de imagem e imagens de handout nunca fizeram parte do backup, então não são restaurados (o Mestre sobe de novo manualmente se precisar). Todo personagem restaurado/clonado nasce sem dono (NPC).
- [x] **Atribuir personagem a um jogador** — cada card de personagem ganhou um seletor "NPC (Mestre)" / lista de jogadores da mesa, pro Mestre dar (ou tirar) o dono de um personagem depois de criado — necessário pra completar o fluxo de restaurar/clonar/instanciar do Bestiário, que sempre criam sem dono. Sem migration (a RLS de `characters` já permitia).
- [x] **Achados da revisão ponta-a-ponta** — depois de simular uma campanha real (Mestre montando tudo + jogador jogando, sem chat) foram corrigidas várias conexões que faltavam entre features já existentes:
  - Toast de revelação de token não fala mais "(Algo)", fala "(Token)".
  - Criar um token vinculado a um personagem que já tem avatar reaproveita a mesma imagem em vez de pedir upload de novo (vale tanto pro formulário manual do mapa quanto pro instanciar do Bestiário).
  - Card de personagem mostra a tag "Sem token no mapa" quando existe um mapa selecionado e aquele personagem ainda não tem token nele.
  - Token de um combatente marcado como "Derrotado" fica dessaturado e com o nome riscado no mapa.
  - **Instanciar do Bestiário já cria o token no mapa atual** (checkbox "Criar token no mapa atual", marcado por padrão quando há um mapa selecionado) — antes o Mestre precisava ir em Tokens e recriar cada um manualmente, o achado de maior impacto da revisão.
  - Formulário de habilidade/item do molde no Bestiário ganhou o mesmo seletor "Do catálogo…" que a Ficha já tinha, reaproveitando as entradas já cadastradas em vez de redigitar.
- [x] **Tabelas de rolagem aleatória** — nova tela **Tabelas**: cria tabelas de sorteio ponderado (texto + peso por entrada) reutilizáveis entre campanhas, pra loot ou encontros aleatórios. Dentro de uma campanha, um widget "Tabelas" na barra lateral (Mestre only) lista as tabelas e sorteia uma entrada por peso ao clicar "Rolar", registrando o resultado no log de Atividade (`🎲 Nome da tabela: resultado`). Requer a migration `018_roll_tables.sql`.
- [x] **Hardening do salvamento de tiles** — pintar um mapa de tiles rápido demais (mais rápido que os 400ms de debounce entre cliques) podia perder parte das mudanças se a aba fosse trocada/recarregada logo em seguida, porque o debounce simples só grava depois de uma pausa que uma pintura contínua nunca dava. Agora existe um teto de espera (força um save a cada 1.5s mesmo sem pausa) e a última mudança pendente é salva na hora ao trocar de mapa, sair da aba Mapa ou fechar a página — e as gravações nunca mais rodam em paralelo (evita que uma gravação mais antiga, atrasada na rede, sobrescreva uma mais nova). Sem migration, só lógica de cliente (`src/lib/debounce.ts`).
